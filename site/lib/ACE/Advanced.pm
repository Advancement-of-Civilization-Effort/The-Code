...
sub get_email {
 my ($user,$n) = @_;
 my $url = sprintf'http://www.yopmail.com/en/?login=%s-%d',$user,$n;
 return $url;
}
...
sub pngdump {
  ...
  our $filename = $basen; $filename =~ y/_/ /; # to display on kst plots
  my $tic = time();
  our $swat = &swat($tic);
  our $date = &hdate($tic);
  our $stamp36 = &stamp36($tic);
  ...
  my $kst2 = 'C:\usr\tools\Kst-2.0.8\ia64\bin\kst2.exe';
  my $pngfile = "$fpath/${basen}_${stamp36}.png";
  system sprintf '"%s" --landscape "%s" --png "%s"',$kst2,$kstfile,$pngfile;
  $pngfile = "$fpath/${basen}_${stamp36}_View 1.png" unless -e $pngfile;
...
sub plot { # using gnuplot to display 2D curves ...
  ...
  close F;
  $pid = open(GP, '| "c:\usr\local\gnuplot\bin\gnuplot.exe" 2>&1 '); 
...
# ----------------------------------------------------
1; #$Source: /my/perl/module/from/ACE/Advanced.pm,v $
